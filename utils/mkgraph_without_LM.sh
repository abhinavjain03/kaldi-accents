...
if [[ ! -s $clg || $clg -ot $lang/L_disambig.fst \
    || ! -s $ilabels || $ilabels -ot $lang/L_disambig.fst ]]; then
  fstcomposecontext --context-size=$N --central-position=$P \
   --read-disambig-syms=$lang/phones/disambig.int \
   --write-disambig-syms=$lang/tmp/disambig_ilabels_${N}_${P}.int \
    $ilabels_tmp < $lang/L_disambig.fst |\
    fstarcsort --sort_type=ilabel > $clg_tmp
  mv $clg_tmp $clg
  mv $ilabels_tmp $ilabels
  fstisstochastic $clg || echo "[info]: CL not stochastic."
fi
