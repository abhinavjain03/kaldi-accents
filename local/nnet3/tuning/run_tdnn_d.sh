...
if [ $stage -le 11 ]; then
  ...
  ivectordev=0
  decodedev=1
  devsets="cv_dev_nz cv_finaltest_nz cv_test_onlynz cv_test_newonlyindian"
  ...
  if [ $decodedev -eq 1 ]; then
    for decode_set in $devsets; do
      ...
      # steps/nnet3/decode.sh --nj $nj --cmd "$decode_cmd" \
      #   --online-ivector-dir /exp/abhinav/accents_multitask_exp/104/ivectors_dev \
      #   $graph_dir /home/abhinav/kaldi/accents_multitask/data/104/dev_hires $dir/decode_cv_dev_nz_onlycanada_hires_sw1_tg || exit 1;
    done
  fi
fi
wait;
